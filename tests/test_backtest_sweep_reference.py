"""Layer 1 sweep (reference engine), no archived Numba sweep."""
...
from src.backtest.sweep import SweepRunConfig, expand_param_grid, run_synthetic_smoke
...
def test_synthetic_sweep_at_least_two_rows():
    df = run_synthetic_smoke()
    assert len(df) >= 2
    assert list(df["combo_id"]) == ["combo_0000", "combo_0001"]


def test_synthetic_sweep_invokes_execution_path():
...
def test_run_single_combo_requires_valid_signal_frame():
    bad = sweep.synthetic_ohlcv_and_signals(target_r=2.0).copy()
