...
- **PA Batch B+C — diversity-repaired reduced Layer 2 v3 (QQQ 2023–2024):** `src/combiner/results/layer2_qqq_pa_batch_bc_repaired_v3_2023_2024/` — configs `layer2_qqq_pa_batch_bc_repaired_v3_2023_2024.yaml` + `layer2_sweep_qqq_pa_batch_bc_repaired_v3_2023_2024.yaml`; **96-combo** sweep + postprocess (top dedupe + cost stress; **behavior-unique** skipped when sweep uses `--detail-top 0`). Summary `layer2_pa_batch_bc_repaired_v3_summary.md` (**`PROCEED_TO_PA_BATCH_BC_MINI_WFO_DESIGN`** — **mini-WFO design only next**; **mini-WFO / full WFO / live not run**).
