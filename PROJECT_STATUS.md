...
**2026-05-11 (fixed-profile OOW v1 — diagnostic):** **`src/research/results/fixed_profile_oow_v1/`** — five fixed combiner YAMLs (VWAP mtp 2/1; indicator mtp 1/2/3), `data_availability.*`, `run_commands_snippet.md`, `postprocess` outputs (`oow/`, `insample_sanity/`, `exit_slip/`, `quality_score_transfer/`, `regime_stability/`, `trade_number/`); **`src/research/fixed_profile_oow.py`**, **`fixed_profile_oow_lib.py`**; **`tests/test_fixed_profile_oow_lib.py`**. **No** strategy / feature / combiner / selected-candidate YAML changes; **no** mini/full WFO / live / SPY / Global L1 / broad L2 grid. **Gate:** **`NEED_MORE_FIXED_PROFILE_OOW`** until `local_runs/` combiner replays exist.
