...
**2026-05-11 (Layer2 candidate robustness v1 — singleton OOW audit):** **`src/research/results/layer2_candidate_robustness_v1/`** — per-candidate combiner replays (**vwap** + **indicator** slice: **27** YAMLs × **early_oow** / **insample_ref** / **late_oow** = **81** runs); curated CSV/MD (labels, family/strategy summaries, failure join vs fixed profiles, policy v2 draft, non-executable side_flip proxy); **`src/research/audit_l2_candidates_oow.py`**, **`audit_l2_candidates_oow_lib.py`**, **`side_flip_diagnostic.py`**; tests `test_audit_l2_candidates_oow.py`, `test_side_flip_diagnostic.py`, `test_l2_core_policy_v2.py`. **No** strategy / feature / combiner / selected-candidate YAML changes; **no** mini/full WFO / live / SPY / Global L1 / broad L2. **Decision:** **`RUN_MORE_CANDIDATE_OOW_AUDIT`**. **`local_runs/` not committed.**
