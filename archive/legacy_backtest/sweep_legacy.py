"""Parameter sweep — **legacy Numba fast grid** (moved from top-level ``sweep.py``).

``python -m src.backtest.sweep --legacy ...`` delegates here. Accounting is
**pre-reset** Numba semantics (``SWEEP_ENGINE = "numba_fast"``); not canonical
``src.execution``.
"""
