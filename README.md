...
- **Global Layer 1 QQQ 2023–2024 v1 (2026-05-10):** Audit `global_strategy_audit_v1/`; curated `layer1_global_qqq_2023_2024_v1/` (all READY strategies with grid ≤1500; **81** strict YAMLs); design `global_layer1_qqq_2023_2024_design.md` + `global_layer2_qqq_2023_2024_design.md`; **`global_branch_leaderboard_v1`**. Conditional Global Layer 2 **not run** (gate: YAML count > 80). Runner: `src/research/run_global_layer1.py`.
- **Global Layer 1 QQQ 2023–2024 v2 (2026-05-11, post feature hardening):** Audit `global_strategy_audit_v2/`; curated `layer1_global_qqq_2023_2024_v2/` (30 sweeps; **81** strict YAMLs); diversity `global_candidate_signal_diversity_qqq_2023_2024_v2/`; **`selected_candidates_l2_core/`** (66 YAMLs via `create_layer2_candidate_core.py`); `global_branch_leaderboard_v2.*`; summaries `global_research_summary_v2.md`, `global_layer2_qqq_2023_2024_v2_design.md`, `global_layer2_gate_decision_v2.md`; combiner configs `layer2_qqq_global_2023_2024_v2.yaml` + sweep. **Full 2023–2024 Layer 2 sweep not committed**; Q1 diagnostics smoke only (`layer2_global_diagnostics_smoke_q1_2023.md`).
- **Feature build performance v1 (2026-05-10):** Batch `pd.concat` for hot-path feature modules (`volume`, `volatility`, `vwap`, `price_action`, `orb`, `levels`); `src/research/results/feature_build_performance_v1/` (before/after benchmarks + summary); `feature_numba_fastpath_design.md` (future Numba kernels, design only). **No** Global Layer 1/2 rerun in this pass.
