"""Stochastic oversold cross — long-only MVP."""
...
def _stoch_os_numba(
    ...
    for i in range(1, n):
        ...
        if past_os[i] == 0:
            continue
        ku = k[i - 1] <= d[i - 1] and k[i] > d[i]
        if not ku:
            continue
        if confirm != 0 and not (close[i] > hh3_prior[i]):
            continue
...
class StochasticOversoldCrossStrategy(BaseStrategy):
    name = "stochastic_oversold_cross"
    supports_fast = True
    ...
    def validate_config(self, config: dict[str, Any]) -> None:
        ...
        validate_int_at_least("features.stoch_window", feat.get("stoch_window", 14), 3)
        validate_int_at_least("features.stoch_smooth", feat.get("stoch_smooth", 3), 1)
        validate_int_at_least("features.swing_lookback", feat.get("swing_lookback", 10), 3)
        cm = str(sig.get("confirm_mode", "k_cross_d"))
        if cm not in ("k_cross_d", "k_cross_d_plus_price_high"):
    ...
    def _kd_cols(self, config: dict[str, Any]) -> tuple[str, str]:
        feat = config.get("features") or {}
        kw = int(feat.get("stoch_window", 14))
        ds = int(feat.get("stoch_smooth", 3))
        return f"stoch_k_{kw}", f"stoch_d_{kw}_{ds}"
    ...
    def context_key(self, config: dict[str, Any]) -> tuple[Any, ...]:
        ...
        return (
            "stoch_os_ctx",
            int(feat.get("stoch_window", 14)),
            int(feat.get("stoch_smooth", 3)),
            int(feat.get("swing_lookback", 10)),
            float(sig.get("oversold_level", 20)),
            str(sig.get("atr_column", "atr_like_20")),
        )

    def prepare_signal_context(self, df: pd.DataFrame, config: dict[str, Any]) -> StochOversoldContext:
        work = df.sort_values("ts_utc", kind="mergesort").reset_index(drop=True)
        kc, dc = self._kd_cols(config)
        for c in (kc, dc):
            if c not in work.columns:
                raise ValueError(f"{self.name}: missing {c!r}; extend stochastic_tuples")
        feat = config.get("features") or {}
        sig = config.get("signal") or {}
        os_lvl = float(sig.get("oversold_level", 20))
        lb = int(feat.get("swing_lookback", 10))
        kser = work[kc].to_numpy(dtype=np.float64)
        pred_os = (work[kc].astype(float) < os_lvl).to_numpy()
        ...
        return StochOversoldContext(
            ...
            k=kser,
            d=work[dc].to_numpy(dtype=np.float64),
    ...
    def generate_signal_arrays_from_context(self, ctx: Any, config: dict[str, Any]) -> dict[str, Any]:
        if not isinstance(ctx, StochOversoldContext):
            raise TypeError(ctx)
        sig = config.get("signal") or {}
        risk = config.get("risk") or {}
        feat = config.get("features") or {}
        es, ee = int(sig["entry_start_minute"]), int(sig["entry_end_minute"])
        confirm = 0 if str(sig.get("confirm_mode", "k_cross_d")) == "k_cross_d" else 1
        ...
        side, valid, stp, tgtp, tmc, tr, rsk = _stoch_os_numba(
            ctx.n,
            ctx.session_id,
            ctx.minute,
            ctx.close,
    ...
    def normalized_param_key(self, config: dict[str, Any]) -> tuple[Any, ...]:
        ...
        return (
            ...
            int(feat.get("swing_lookback", 10)),
            float(sig.get("oversold_level", 20)),
            str(sig.get("confirm_mode", "k_cross_d")),
