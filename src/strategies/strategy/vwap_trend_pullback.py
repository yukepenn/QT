"""VWAP trend pullback — true Numba fast core."""
...
class VwapTrendPullbackContext:
    n: int
...
def _vwap_trend_numba(
    ...
    for i in range(n):
        if minute[i] < es or minute[i] > ee:
            continue
        if pers[i] < min_p or slope[i] <= 0.0:
            continue
        if low[i] > vw[i] + pull_tol * atr[i]:
            continue
        if close[i] < vw[i]:
            continue
        if req_dry != 0 and volr[i] > max_vol:
            continue
        ok = True
        if confirm == 1:
            ok = close[i] > ph[i]
        elif confirm == 2:
            ok = (close[i] > pc[i]) and (cloc[i] >= 0.55)
        if not ok:
            continue
        cand_long[i] = True
...
class VwapTrendPullbackStrategy(BaseStrategy):
    """VWAP trend pullback — long-only MVP."""
    ...
    def validate_config(self, config: dict[str, Any]) -> None:
        ...
        validate_int_at_least("signal.trend_window", sig.get("trend_window", 60), 1)
        validate_nonnegative_number("signal.min_vwap_persistence", sig.get("min_vwap_persistence", 0.7))
        validate_nonnegative_number("signal.pullback_tolerance_atr", sig.get("pullback_tolerance_atr", 0.5))
        confirm = str(sig.get("confirm_mode", "close_reclaim"))
        if confirm not in ("close_reclaim", "break_trigger_high", "momentum_turn"):
            raise ValueError(f"signal.confirm_mode invalid: {confirm!r}")
        stop_mode = str(risk.get("stop_mode", "pullback_low"))
        if stop_mode not in ("pullback_low", "vwap_buffer", "swing_low"):
    ...
    def context_key(self, config: dict[str, Any]) -> tuple[Any, ...]:
        sig = config.get("signal") or {}
        tw = int(sig.get("trend_window", 60))
        return ("vwap_trend_ctx", tw, str(sig.get("atr_column", "atr_like_15")))

    def prepare_signal_context(self, df: pd.DataFrame, config: dict[str, Any]) -> VwapTrendPullbackContext:
        work = df.sort_values("ts_utc", kind="mergesort").reset_index(drop=True)
        sig = config.get("signal") or {}
        tw = int(sig.get("trend_window", 60))
        col_p = f"vwap_persistence_above_{tw}"
        if col_p not in work.columns:
            col_p = "vwap_persistence_above_60"
        slope_col = f"vwap_slope_{tw}" if f"vwap_slope_{tw}" in work.columns else "vwap_slope_20"
        session_id = session_id_from_dates(work["session_date"])
        low = work["low"].to_numpy(dtype=np.float64)
        rl5 = rolling_min_by_session(low, session_id, 5)
        rl10 = rolling_min_by_session(low, session_id, 10)
        atr = atr_series(work, config).to_numpy(dtype=np.float64)
        return VwapTrendPullbackContext(
            n=len(work),
            ...
            persistence=work[col_p].to_numpy(dtype=np.float64),
            slope=work[slope_col].to_numpy(dtype=np.float64),
            atr=atr,
            roll_lo5=rl5,
            roll_lo10=rl10,
        )
    ...
    def generate_signal_arrays_from_context(self, ctx: Any, config: dict[str, Any]) -> dict[str, Any]:
        if not isinstance(ctx, VwapTrendPullbackContext):
            raise TypeError(ctx)
        sig = config.get("signal") or {}
        risk = config.get("risk") or {}
        ...
        pull_tol = float(sig.get("pullback_tolerance_atr", 0.5))
        confirm = str(sig.get("confirm_mode", "close_reclaim"))
        req_dry = int(bool(sig.get("require_volume_dryup", False)))
        max_vol = float(sig.get("max_pullback_volume_ratio", 0.8))
        stop_mode = str(risk.get("stop_mode", "pullback_low"))
        ...
        cmap = {"close_reclaim": 0, "break_trigger_high": 1, "momentum_turn": 2}
        ci = cmap.get(confirm, 0)
        smap = {"pullback_low": 0, "vwap_buffer": 1, "swing_low": 2}
        sm = smap.get(stop_mode, 0)
        side, valid, stp, tgtp, tmc, tr, rsk = _vwap_trend_numba(
            ctx.n,
            ctx.session_id,
            ctx.minute,
            ctx.close,
    ...
    def generate_signals(self, df: pd.DataFrame, config: dict[str, Any]) -> pd.DataFrame:
        ...
        out.loc[out["sig_valid"], "sig_reason"] = "vwap_trend_pullback_long"
    ...
    def normalized_param_key(self, config: dict[str, Any]) -> tuple[Any, ...]:
        ...
        return (
            ...
            float(sig.get("pullback_tolerance_atr", 0.5)),
