...
- **Exit overlay diagnostics v2 (2026-05-11)** — `exit_overlay_diagnostics_v2/` (`baseline_inventory.md`, `docs_sync_inventory.*`, `combiner_semantics_inventory.*`, `replay_drift_hypotheses.*`, `alignment/*` incl. `alignment_best_config.yaml` + grid CSV/MD, `overlay_v2/*` aggregates + context splits + ambiguity sensitivity + `overlay_v2_config.yaml` + `overlay_v2_design.*`, `exit_vs_router_quality_v2_comparison.*`, `scalp_short_after_exit_overlay_v2.*`, `exit_overlay_diagnostics_v2_{decision,summary}.md`, `CHATGPT_REVIEW_BUNDLE.md`, `SOURCE_MAP.csv`, `chatgpt_key_tables.csv`, `exit_overlay_diagnostics_v2_artifact_validation.*`); scripts `src/research/exit_overlay_alignment.py`, `src/research/run_exit_overlay_diagnostics_v2.py`, `exit_overlay_sim.py` updates; tests `tests/test_exit_overlay_alignment.py`, `tests/test_run_exit_overlay_diagnostics_v2.py`. **Uses** same local-only panel + parquet as v1; **curated CSVs may include synthetic schema-smoke rows** — re-run CLI locally for full economics. **No** production exit-management/router; **no** WFO/live/SPY/broad L2; **no** strategy/feature/YAML edits. **Decision (curated):** **`REFINE_REPLAY_ALIGNMENT`**.
  - **keep:** yes
