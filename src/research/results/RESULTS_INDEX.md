...
- **Exit overlay diagnostics v2 (2026-05-11)** — `exit_overlay_diagnostics_v2/` (**real full-panel alignment**: 10,628 rows, 617k QQQ bars; `alignment/*`, `full_panel_alignment_manifest.csv`, `full_panel_alignment_failure_*`, `alignment/archive_synthetic_pre_full_panel/`; **`--mode overlay` not run** — `overlay_v2/overlay_v2_summary.md`, `overlay_v2/full_panel_overlay_manifest.csv`; `full_panel_run_inventory.*`, `local_full_panel_input_check.*`, `full_panel_commands.*`, `comparison_not_run_reason.md`, `exit_overlay_diagnostics_v2_{decision,summary}.md`, `CHATGPT_REVIEW_BUNDLE.md`, `SOURCE_MAP.csv`, `chatgpt_key_tables.csv`, `exit_overlay_diagnostics_v2_artifact_validation.*`, plus baseline/combiner_semantics/replay drift files); scripts `src/research/exit_overlay_alignment.py`, `src/research/run_exit_overlay_diagnostics_v2.py`, `exit_overlay_sim.py` updates; tests `tests/test_exit_overlay_alignment.py`, `tests/test_run_exit_overlay_diagnostics_v2.py`. **Uses** local-only panel; **does not** commit `local_rows/**`. **Decision:** **`REFINE_REPLAY_ALIGNMENT`** (max_hold vs replay stop/target path drift).
  - **keep:** yes
