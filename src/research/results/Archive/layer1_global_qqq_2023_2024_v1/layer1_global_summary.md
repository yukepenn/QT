...
## Layer 2 prerun snapshot

- Global conditional gate: **NO** — strict YAML count **81** exceeds design cap **80** by one (see `global_layer2_gate_decision.md`).
- **15** distinct `strategy_family` values among strict-selected rows (families gate **passes**).
- Fast-context smoke (2023-01-03 → 2023-01-31): **all `ok`**.
- Diversity tooling: **exit 0**; **22** duplicate-signal group rows (readability heuristic **passes**).

### Heuristic branch labels (in-sample Layer 1 only)

From `global_branch_leaderboard_v1.csv`: **17** strategies exported at least one strict YAML; **13** strategies had sweeps but **no** row passed strict filters (`DEFER_NO_CANDIDATE` for Layer 2 purposes).

Recommended design-time decision: **`TUNE_GLOBAL_LAYER1_OR_BUCKETS`** — trim selection width (for example `--top-per-strategy 4`) or tighten one filter, then re-run gate before any Global Layer 2 execution.
