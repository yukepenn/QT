...
- **Exit overlay diagnostics v1 (2026-05-11)** — `exit_overlay_diagnostics_v1/` (`baseline_inventory.md`, `docs_sync_inventory.*`, `local_input_inventory.*`, `exit_overlay_harness_design.{md,csv}`, `exit_overlay_config_v1.yaml`, `overlay_results_by_profile.csv`, `overlay_results_by_window.csv`, `overlay_results_detail_by_profile_window.csv`, `overlay_results_by_{candidate,context,market_context,trade_number,exit_reason}.csv`, `overlay_weak_period_results.csv`, `overlay_ambiguity_summary.csv`, `overlay_sanity_vs_original.csv`, `overlay_summary.md`, context CSV/MD, `exit_vs_router_quality_comparison.*`, `scalp_short_after_exit_overlay.*`, `exit_overlay_diagnostics_{decision,summary}.md`, `CHATGPT_REVIEW_BUNDLE.md`, `SOURCE_MAP.csv`, `chatgpt_key_tables.csv`, `dry_run_*`, `exit_overlay_diagnostics_artifact_validation.*`); scripts `src/research/exit_overlay_sim.py`, `src/research/run_exit_overlay_diagnostics.py`; tests `tests/test_exit_overlay_sim.py`, `tests/test_run_exit_overlay_diagnostics.py`. **Uses** gitignored `local_detailed_trade_context_replay_v1/local_rows/trade_context_panel.csv` + optional **`exit_overlay_diagnostics_v1/local_rows/overlay_trade_results.csv`**; **does not** commit row-level outputs. **No** WFO/live/SPY/broad L2/Global L1; **no** production exit-management/router wiring; **no** strategy/feature/YAML edits. **Decision:** **`RUN_EXIT_OVERLAY_DIAGNOSTICS_V2`** (replay vs panel drift; refine simulator then reconsider integration vs router path).
  - **keep:** yes

- **Exit overlay diagnostics v2 (2026-05-11)** — `exit_overlay_diagnostics_v2/` (**real full-panel alignment**: 10,628 rows, 617k QQQ bars; `alignment/*` **18 configs** incl. `max_hold_priority` variants, `full_panel_alignment_manifest.csv`, `full_panel_alignment_failure_*`, `max_hold_alignment_v1/*` curated aggregates + `build_max_hold_alignment_v1_aggregates.py`; **`--mode overlay` not run** — `overlay_v2/overlay_v2_summary.md`, `overlay_v2/full_panel_overlay_manifest.csv`; `full_panel_run_inventory.*`, `local_full_panel_input_check.*`, `full_panel_commands.*`, `comparison_not_run_reason.md`, `exit_overlay_diagnostics_v2_{decision,summary}.md`, `CHATGPT_REVIEW_BUNDLE.md`, `SOURCE_MAP.csv`, `chatgpt_key_tables.csv`, `exit_overlay_diagnostics_v2_artifact_validation.*`); scripts `src/research/exit_overlay_alignment.py`, `src/research/run_exit_overlay_diagnostics_v2.py`, `exit_overlay_sim.py`, `build_max_hold_alignment_v1_aggregates.py`; tests `tests/test_exit_overlay_alignment.py`, `tests/test_run_exit_overlay_diagnostics_v2.py`. **Uses** local-only panel; **does not** commit `local_rows/**`. **Decision:** **`REFINE_REPLAY_ALIGNMENT`** (pre-terminal max_hold drift vs replay).
  - **keep:** yes
...
- **Local detailed trade-context replay v1 (2026-05-11, executed local-only rows + committed aggregates)** — `local_detailed_trade_context_replay_v1/` (local-only `local_runs/**`, `local_rows/**` row panel; committed `aggregates/`, `router_diagnostics_v1/`, `quality_score_v2/`, `attribution_v1/`, `freshness_v1/`, `exit_overlay_readiness_v1/`, `roadmap_update_v1/`, `CHATGPT_REVIEW_BUNDLE.md`, `SOURCE_MAP.csv`, decision). Scripts: `src/research/run_local_trade_context_replay.py`, `run_offline_router_diagnostics.py`, `run_trade_quality_score_v2.py`, `analyze_local_trade_context_panel.py`. **No** WFO/live/SPY/broad L2; no strategy/feature/YAML/production router changes; no row-level artifacts committed.
  - **keep:** yes
