...
def write_candidate_diagnostics(
    ...
    *,
    ...
    """Write overlap / conflict CSVs (Python/NumPy)."""
    ...
    pairs = []
    session_date = csm.meta_arrays["session_date"]
    for ci in range(nc):
        if not mask[ci]:
            continue
        for cj in range(ci + 1, nc):
            if not mask[cj]:
                continue
            v1 = csm.valid[ci] & (csm.side[ci] != 0)
            v2 = csm.valid[cj] & (csm.side[cj] != 0)
            same_bar = int(np.sum(v1 & v2))
            sd_arr = np.asarray(session_date)
            d1 = set(np.unique(sd_arr[v1]))
            d2 = set(np.unique(sd_arr[v2]))
            same_day = len(d1 & d2)
            opp = int(np.sum(v1 & v2 & (csm.side[ci] != csm.side[cj]) & (csm.side[ci] != 0) & (csm.side[cj] != 0)))
            same_dir = int(np.sum(v1 & v2 & (csm.side[ci] == csm.side[cj])))
            a = csm.candidates[ci]
            b = csm.candidates[cj]
            mdiffs = []
            ix1 = np.flatnonzero(v1)
            ix2 = np.flatnonzero(v2)
            if len(ix1) and len(ix2):
                # approximate same-day minute diff distribution
                for d in np.unique(session_date[v1 | v2]):
                    m1 = minute[(session_date == d) & v1]
                    m2 = minute[(session_date == d) & v2]
                    if len(m1) == 0 or len(m2) == 0:
                        continue
                    mdiffs.append(float(np.min(np.abs(m1.reshape(-1, 1) - m2.reshape(1, -1)))))
            avg_md = float(np.mean(mdiffs)) if mdiffs else np.nan
            pairs.append(
                {
                    ...
                    "family_a": a.family,
                    "family_b": b.family,
                    "same_bar_overlap": same_bar,
                    "same_day_overlap": same_day,
                    "opposite_side_same_bar": opp,
                    "same_direction_same_bar": same_dir,
                    "avg_abs_minute_diff_on_same_day": avg_md,
                }
            )
    pd.DataFrame(pairs).to_csv(out_dir / "candidate_conflict_summary.csv", index=False)

    mat = np.zeros((nc, nc), dtype=np.int32)
    for ci in range(nc):
        if not mask[ci]:
            continue
        for cj in range(nc):
            ...
            v1 = csm.valid[ci] & (csm.side[ci] != 0)
            v2 = csm.valid[cj] & (csm.side[cj] != 0)
            mat[ci, cj] = int(np.sum(v1 & v2))
    ids = [csm.candidates[i].candidate_id for i in range(nc)]
    pd.DataFrame(mat, index=ids, columns=ids).to_csv(out_dir / "candidate_overlap_matrix.csv")
