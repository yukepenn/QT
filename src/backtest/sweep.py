...
- ``--validate-pipeline`` — strategy metadata and optional bars→features→signals checks (no backtest).
- Real-symbol sweep — ``--strategy``, ``--symbol``, ``--start``, ``--end``, ``--data-root`` together; optional ``--config``, ``--grid``, ``--dry-run``, ``--output-root``.
...
from src.backtest.legacy.engine_legacy import BacktestConfig, _bt_cfg_from_dict
...
def run_single_canonical_from_signals(
    ...
    *,
    ...
    """Run canonical backtest on a full OHLCV + ``sig_*`` frame (real-symbol path)."""
    ...
    tdf, summ = run_strategy_backtest(signal_df, config=bt, policy=pol, exit_plan=None)
    tg = float(summ.get("total_gross_r", float("nan")))
    if tg != tg:
        tg = float("nan")
    pfr = float(summ.get("profit_factor_r", float("nan")))
...
def run_canonical_real_symbol_sweep(
    *,
    strategy_name: str,
    symbol: str,
    asset: str,
    ...
    """Load bars once; for each grid combo build features → signals → canonical backtest."""
    from src.backtest.legacy import sweep_legacy as sl

    base_cfg = load_strategy_config_merged(strategy_name, config_path, None)
    ...
    for i, combo in enumerate(combos):
        ...
        sl._finalize_combo_config(cfg)
        fch = feature_config_fingerprint(cfg)
        _, canon = run_strategy_pipeline(
        ...
        cid = f"combo_{i:04d}"
        sr = run_single_canonical_from_signals(
            canon,
            combo_id=cid,
            combo_params=combo,
...
def _write_real_sweep_artifacts(
    df: pd.DataFrame,
    out_dir: Path,
    manifest: dict[str, Any],
) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)
    df.to_csv(out_dir / "canonical_sweep_results.csv", index=False)
    ...
    lines = [
        ...
        f"- combos: {len(df)}",
        "",
        "Per-combo metrics are in `canonical_sweep_results.csv`.",
    ]
    (out_dir / "canonical_sweep_summary.md").write_text("\n".join(lines), encoding="utf-8")
...
def canonical_sweep_main(argv: list[str] | None) -> int:
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Directory for canonical_sweep_results.csv, summary.md, run_manifest.json.",
    )
    ...
    if args.data_root and not args.smoke and not args.validate_pipeline and not want_real:
        print(
            ...
            "or use --smoke / --validate-pipeline / --legacy.",
            file=sys.stderr,
        )
        return 3
    ...
    if args.canonical_help:
        print(
            "Canonical sweep: read_bars → build_features_from_config → generate_signals → "
            "canonical sig_* → run_strategy_backtest → simulate_trade_path → summarize_trades. "
            "Use --smoke for synthetic regression; --validate-pipeline to check wiring; "
            "--legacy must be argv[0] for Numba grid (separate accounting)."
        )
        return 0
    ...
    if want_real:
        ...
        if args.output_root and not args.no_save:
            ...
            manifest: dict[str, Any] = {
                ...
                "engine": CANONICAL_ENGINE_LABEL,
                "execution_semantics_version": str(pol.semantics_version),
                "symbols": args.symbol,
                "strategy": args.strategy,
                "start": args.start,
                "end": args.end,
            ...
            _write_real_sweep_artifacts(df, Path(args.output_root), manifest)
