...
| `src/combiner/adapter.py` | Canonical sequential Layer2 loop → `simulate_trade_path` |
| `src/combiner/trade_intent_adapter.py` | `TradeIntent` construction + `TradeResult` → trade row |
| `src/combiner/simulator.py` | `CombinerConfig`, rejection/exit constants; lazy legacy + `simulate_combiner_canonical` |
