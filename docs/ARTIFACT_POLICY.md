...
- Candidate libraries:
  ...
  - **PA Batch B/C gate + exit diagnostics v1:** `pa_batch_bc_gate_diagnostics_v1/`, `pa_batch_bc_exit_diagnostics_v1/` — compact CSV/MD + scripts `pa_gate_diagnostics.py`, `pa_exit_diagnostics.py` (design aid before tuned v2 grids; **not** a Layer 1 rerun)
