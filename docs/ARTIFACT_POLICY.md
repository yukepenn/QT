...
- Candidate libraries:
  ...
  - **PA Batch B/C gate + exit diagnostics v1:** `pa_batch_bc_gate_diagnostics_v1/`, `pa_batch_bc_exit_diagnostics_v1/` — compact CSV/MD + scripts `pa_gate_diagnostics.py`, `pa_exit_diagnostics.py`
  - **PA Batch B/C tuned v2 Layer 1:** `layer1_pa_batch_bc_tuned_qqq_2023_2024_v2/` — `sweep_manifest.*`, per-strategy `sweep_*/` under research root (results CSV + summary; not `testing_parameters_results/`), `selected_candidates/`, `signal_rate_diagnosis.*`, `candidate_fast_context_check.*`, `pa_batch_bc_gate_diagnostics_v2_preflight/`, `pa_batch_bc_exit_diagnostics_v2/`; design-only `reduced_layer2_pa_batch_bc_tuned_v2_design.md`
